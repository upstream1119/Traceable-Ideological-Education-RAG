...
@lru_cache(maxsize=1)
def _load_demo_graph_store() -> GraphStore | None:
    if not DEMO_TRIPLES_PATH.exists():
        return None
    return GraphStore.from_jsonl(DEMO_TRIPLES_PATH)
...
def retrieve_graph(query_entities: list[str], top_k: int = GRAPH_TOP_K) -> list[dict]:
    ...
    """
    graph_store = _load_demo_graph_store()
    if graph_store is None:
        return []
    return graph_store.search(query_entities, top_k=top_k, max_hops=2)
