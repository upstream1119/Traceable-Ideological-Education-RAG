...
def _rerank_hits_by_query_terms(query: str, hits: list[dict]) -> list[dict]:
    reranked = []
...
def _write_summary(
    ...
    lines.extend(
        [
            ...
            "- 默认索引语料包含 text_chunks_sizheng_v1.jsonl 与 text_chunks_sizheng_v2.jsonl。",
            "- 本轮用于验证统一知识库的真实 Embedding + FAISS 检索效果。",
...
def main() -> None:
    ...
    for query_item, query_vector in zip(queries, query_vectors):
        ...
        hits = _rerank_hits_by_query_terms(query_item["query"], hits)
